...
function fillExercise(name, serie) {
    ...
    for (let i = 0; i < serie.length; i++) {
        li.innerHTML = li.innerHTML + completeElement(name, i + 1, serie[i].carico, serie[i].reps)
    }
...
function completeElement(name, num, carico, reps) {
    ...
    let exElement =
        ...
        '<input type="checkbox" class="custom-control-input" id="' + nameToPlace + num + '">' +
        '<label class="custom-control-label ' + nameToPlace + 'Serie' + '" for="' + nameToPlace + num + '">' +
        '<kbd>' + carico + '</kbd>\n' +
        '<kbd>' + reps + '</kbd>\n' +
        '</label></div>' +
...
function fillButtons(json) {
    ...
    json.schede.forEach(scheda => {
        ...
        let button = '<button type="button" class="btn btn-outline-secondary schedaButton">' + name + '</button>'
