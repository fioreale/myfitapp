...
function updateScheda(workout_id) {
    sendUpdate.onclick = function() {
        ...
        let scheda_name = document.querySelector('.listExercises').getAttribute('scheda')
        ...
        if (scheda_name !== '') {
            ...
            exercisesArray.forEach(el => {
                ...
                let arraySerie = document.querySelectorAll('.' + exerciseName.replace(/[^a-zA-Z0-9]+/g, '') + 'Serie')
            ...
            fetch('../workout/' + workout_id, {
                ...
                .then(function(response) {
                    ...
                    fillPage(workout_id)
                    sendUpdate.parentElement.setAttribute('hidden', '')
                    document.querySelector('#alert-text').textContent = 'Aggiornamento eseguito!'
